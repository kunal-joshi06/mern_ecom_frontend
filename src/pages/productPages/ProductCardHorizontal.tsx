    ...
    return (
        <div className="w-full h-full p-4 flex flex-row justify-between bg-white border-2 border-gray-100 rounded-lg shadow-md">
           <div className="min:h-full min:w-full flex justify-center items-center">
                <img className="object-contain" src={product?.imageUrl} alt="product image" />
           </div>
            <div className="pl-5 flex flex-col min:h-full min:w-full">
                ...
                <div className="flex items-center mt-2.5 mb-5">
                   <StarRating rating={product?.rating} />
                    <span className="bg-blue-100 text-blue-800 text-xs font-semibold mr-2 px-2.5 py-0.5 rounded dark:bg-blue-200 dark:text-blue-800 ml-3">{Math.ceil(product?.rating)}</span>
                </div>
                <div className="flex items-center justify-between">
                    <span className="text-lg font-bold text-gray-500">₹{product?.price}</span>
                    <Link to={product._id} className="text-white bg-blue-500 hover:bg-blue-700 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">View</Link>
                </div>
                </div>
